...
extern "C" volatile bool	 apic_wait_isr_done[MAX_CORE_COUNT]	  = {0};
extern "C" volatile uint32_t apic_wait_loop_count[MAX_CORE_COUNT] = {0};
extern "C" volatile uint32_t apic_wait_loop_max[MAX_CORE_COUNT]	  = {0};
...
void start_timer_count(uint8_t vector, uint64_t count, enum timer_mode::type timer_mode, mask::type mask = mask::enable)
{
	...
	for (auto configuration : configurations)
	{
		...
		{
			start_timer(vector, static_cast<uint32_t>(timer_count), configuration, timer_mode, mask);
...
{
	lapic.initial_count.write({.value = 0});
}

void mask_timer()
{
	lapic.lvt_timer.write({.mask = mask::disable});
}

void unmask_timer()
{
	lapic.lvt_timer.write({.mask = mask::enable});
}
...
void quick_wait(uint32_t count)
{
	...
	apic_wait_isr_done[core_id] = false;
	start_timer_count_single_shot(apic_wait_interrupt, count);
	while (!apic_wait_isr_done[core_id])
	{
...
void quick_wait_n_times(uint32_t count, uint32_t n)
{
	...
	start_timer_count_periodic(apic_wait_interrupt, count);

	while (apic_wait_loop_max[core_id] < n)
	{
		// Wait and poll for it to be done
	}
	stop_perioic_timer();
