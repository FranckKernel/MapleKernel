...
void multicore_setup(void *rsdp_void)
{
	...
	kprintf("Apic_wait::wait(5) : Waiting 5 seconds using apic!\n");
	apic_wait::wait(5);
	kprintf("It has been 5 seconds!\n");
	apic_wait::wait(0.5);
...
#
	...
	// apic_wait::wait(10);
	// apic_wait::quick_wait_n_times(UINT32_MAX, 2);
	apic_wait::quick_wait(UINT32_MAX);
	terminal_writestring("\nAfter apic wait\n");
