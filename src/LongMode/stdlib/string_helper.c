...
size_t uitoa(uint64_t value, char *buffer)
{
	size_t pos = 0;
...
uint64_t string_to_uint(const char *str)
{
	uint32_t power = 1;
	uint32_t sum   = 0;
...
size_t uint_to_hex(uint64_t value, char *buffer, int uppercase)
{
	...
	char temp[17]; // Max 8 hex digits + null terminator
	temp[16] = '\0';
